...
<head>
    ...
    <title>Distributor-Gangotri Ganga</title>
                                                    ...
                                                    <form class="row g-3" method="post" enctype="multipart/form-data">
                                                        <div class="col-md-6">
                                                            ...
                                                            <input type="hidden" name="rand"
                                                                value="<?php echo "DIST_INVT_PRODUCT_" . sprintf('%04d', rand(1, 9999)); ?>">
                                                        </div>
...
if (isset($_POST['submit'])) {
    ...
    $result = mysqli_query($conn, "SELECT MAX(SUBSTRING_INDEX(prod_id, '_', -1))AS max_id FROM distributor_prod_inventory");
    $row = mysqli_fetch_assoc($result);
    $max_id = $row['max_id'];
    $next_id = $max_id + 1;
    $rand = "DIST_INVT_PRODUCT_" . sprintf('%04d', $next_id);


    // Corrected SQL Query
    $sql = "INSERT INTO `distributor_prod_inventory`(`prod_id`, `prod_name`, `inven_cat`, `date`, `prod_qty`, `batch_code`, `prod_expiry`) VALUES ('$rand','$pro_name','$inven_cat','$date','$pro_qty','$batch_code','$prod_expiry')";
