...
<body class="ec-header-fixed ec-sidebar-fixed ec-sidebar-dark ec-header-light" id="body">
                                        ...
                                        <div class="col-lg-12 col-md-12 col-sm-12">
                                            <div class="ec-vendor-upload-detail">
                                                <form class="row g-3" method="post" enctype="multipart/form-data">
                                                    ...
                                                    <!-- Add an onchange event to the retailer dropdown -->
                                                        <div class="col-md-6">
                                                            <label class="form-label">Assign to Retailer*</label>
                                                            <select id="Categories" class="form-select" name="retailer" onchange="fetchRetailerId(this.value)">
                                                                <option value="">Choose Retailer</option>
                                                                <?php
                                                                $query = "SELECT DISTINCT branch_add2 FROM `gkb_branch_detail`";
                                                                $qury_run = mysqli_query($conn, $query);
                                                                while ($roow = mysqli_fetch_assoc($qury_run)) {
                                                                ?>
                                                                    <option value="<?php echo $roow['branch_add2']; ?>"><?php echo $roow['branch_add2']; ?></option>
                                                                <?php
                                                                }
                                                                ?>
                                                            </select>
                                                        ...
                                                        <div class="col-md-6">
                                                            ...
                                                            <div id="retailerIdContainer" class="border rounded p-3">
                                                                <!-- Retailer Id will be displayed here -->
                                                            </div>
                                                        </div>
 ...
 <script>
     function fetchRetailerId(selectedRetailer) {
        // Check if a retailer is selected
            if (selectedRetailer !== "") {
            // Use AJAX to fetch the corresponding Retailer Id
            $.ajax({
            type: "POST",
            url: "fetch_retailer_id.php", // Replace with the actual server-side script
            data: { retailer: selectedRetailer },
            success: function(response) {
             // Update the Retailer Id field
            $("#retailerIdContainer").html(response);
            }
            });
            } else {
            // Clear the Retailer Id field if no retailer is selected
            $("#retailerIdContainer").html("");
        }
            }
</script>                                                       
...
if (isset($_POST['save'])) {
    ...
    // Corrected SQL Query
    $sql = "UPDATE `orders` SET `name`='$name',`phone`='$phone',`address`='$address',`pmode`='$pmode',`products`='$products',`order_id`='$order_id',`amount_paid`='$amount_paid',`retailer`='$retailer', `date`='$date' WHERE `id` = '$id'";
